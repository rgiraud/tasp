...
//TASP superpixel segmentation algorithm

void TASP(unsigned char* R,unsigned char* G,unsigned char* B,int nRows,int nCols,
        int superpixelnum, float ratio,int* label, int patch_w, int Kpm)
{
    //LSC feature space parameters
    ...
    int newSeedNum=Seeds(nRows,nCols,RowNum,ColNum,StepX,StepY,seedArray);
    ...
    DoSuperpixel(L1,L2,a1,a2,b1,b2,x1,x2,y1,y2,W,label,seedArray,newSeedNum,nRows,nCols,StepX,StepY,iterationNum,
            thresholdCoef, R, G, B, patch_w, Kpm);
    delete []seedArray;
